...
Node* AVLSet(Node* node)
{
	...
	if (Depth >= 2)//Left
	{
		...
		if (ChildDepth >= 1)
		{
			Node* temp;
			//LTL
			printf("1. temp에 Dnode의 주소를 넣음\n");
			temp = node->left;
			printf("2. Bnode의 left에 Dnode의 right의 주소를 넣음\n");
			node->left = temp->right;
			printf("3. temp의 right에 Bnode의 주소를 넣음\n");
			temp->right = node;
			printf("4. temp에 Dnode의 주소를 넣음\n");
			temp->parent = node->parent;
			printf("5. Bnode의 parent의 주소를 temp로 바꿈\n");
			node->parent = temp;
			printf("6. temp의 parentdml left는 temp\n");
